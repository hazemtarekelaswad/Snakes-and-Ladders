...
void Output::CreatePlayModeToolBar() const
{
	...
	MenuItemImages[ITM_ROLL_DICE] = "images\\dice.jpg";
	MenuItemImages[ITM_DICE_VALUE] = "images\\specialDice.jpg";
	MenuItemImages[ITM_ACCEPT_MORTGAGE] = "images\\accept.jpg";
	MenuItemImages[ITM_OFFER_MORTGAGE] = "images\\offer.jpg";
	MenuItemImages[ITM_RETURN_MORGAGE] = "images\\return.jpg";
	MenuItemImages[ITM_SELL_CITY] = "images\\sell.jpg";
	MenuItemImages[ITM_NEW_GAME] = "images\\new.jpg";
	MenuItemImages[ITM_PLAY_EXIT] = "images\\exit.jpg";
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell . IsValidCell() &&  toCell . IsValidCell() && toCell.GetCellNum() > fromCell.GetCellNum())
	{
		...
		/*for (int y = fromStartY ; y <=  (toStartY + UI.CellHeight) ; y -= UI.CellHeight)
		{
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if(fromCell.IsValidCell() &&  toCell.IsValidCell() && toCell.GetCellNum() < fromCell.GetCellNum())
	{
		...
		int px[]= {x12 + xChange , x12 , x12 - xChange, x12}; // Can be modified 
		int py[]= {y1, y1 + yChange, y1, y1 - yChange};
...
void Output::DrawCoinSet(const CellPosition & cellPos) const
{
	...
	pWind->DrawImage("images\\coins.jpg", GetCellStartX(cellPos), GetCellStartY(cellPos), UI.CellWidth, UI.CellHeight);
