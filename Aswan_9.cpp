...
Aswan_9::Aswan_9(const CellPosition &position)
	:Card(position){
	cardNumber = 9;
	m_cityOwner = NULL;
...
void Aswan_9::ReadCardParameters(Grid *pGrid){
	...
	if(pGrid->GetFirstCard(cardNumber)){
		...
		Aswan_9* firstCard = dynamic_cast<Aswan_9*>(pGrid->GetFirstCard(cardNumber));
		m_cityPrice = firstCard->m_cityPrice;
		m_rent = firstCard->m_rent;
		outPtr->ClearStatusBar();
		return;
	}
	outPtr->PrintMessage("New Monopoly Card (9) | Aswan City | Enter its Price: ");
	m_cityPrice = inPtr->GetInteger(outPtr);

	outPtr->PrintMessage("Aswan City | Enter its Rent: ");
...
void Aswan_9::Apply(Grid *pGrid, Player *pPlayer){
	...
	if(!m_cityOwner){
		string option;
		do{
			outPtr->PrintMessage("Do you want to buy Aswan City? .. (y | n)");
...
void Aswan_9::setCityOwner(Player* pPlayer){
	m_cityOwner = pPlayer;
}

Player* Aswan_9::GetCityOwner() const{
	return m_cityOwner;
}

void Aswan_9::SetOriginalOwner(Player* pPlayer){
	m_originalOwner = pPlayer;
}

Player* Aswan_9::GetOriginalOwner() const{
	return m_originalOwner;
}

int Aswan_9::GetCityPrice() const{
	return m_cityPrice;
}

int Aswan_9::GetCityRent() const{
	return m_rent;
}

Aswan_9::~Aswan_9(){}

void Aswan_9::Save(ofstream& outFile, Type gameObj)
{
	if (gameObj == CARDS)
	{
...
void Aswan_9::Load(ifstream& InFile){
	Card::Load(InFile);
	if(isFirst){
		int price;
		int rent;
